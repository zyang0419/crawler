...
import com.jin.crawler.mongo.BlogMongo;
 ...
 *@return
 */
public class SinaBlogPageMongoSaveProcessor implements PageProcessor {
    ...
    public SinaBlogPageMongoSaveProcessor(){

    }


    public SinaBlogPageMongoSaveProcessor(String projectId, String projectName, String keyWord){
    ...
    public void process(Page page){
        try {
            ...
            for(int i=0;i<nodes.size();i++) {
                 ...
                 * 插入前先判断是否已经爬取过
                 */
                List<BlogMongo> existList = (List<BlogMongo>) MongodbUtils.find(blog,new String[]{"link"},new String[]{link});
                if(CollectionUtils.isEmpty(existList)){
                    MongodbUtils.save(blog);
                }
...
//        Spider.create(new GithubRepoPageProcessor()).addUrl("https://github.com/code4craft").thread(5).run();
        ...
        Spider.create(new SinaBlogPageMongoSaveProcessor()).setDownloader(new HttpClientDownloader()).addUrl(url).thread(1).run();
    }
}